...
| **Asking what an article field IS, or where a blocked article went** | **`NexusMind/contracts/article-record.schema.json`** — the record's definition since 2026-08-24, owner-ruled **prescriptive** and **composed** (`allOf: [$ref Contract A, nexusmind_layer]`), so producer fields have one definition and cannot drift. Human half: `NexusMind/docs/ARTICLE_RECORD.md`. ⛔ **Do not quote a field count from any of them** — "132" was a 2-cycle window and the same script says 212 at 12 cycles; print the window with the number or say nothing. ⛔ **"The layer is declared nowhere" is FALSE** — the genuinely undeclared set is 20 fields. ⚠️ The census that generates the register has three mis-defined columns (NM#401): `pop%` is `populated/seen`, `distinct` is censored at 13, readers is a bare leaf-name grep. Blocked articles: `docs/BLOCK_LEDGER_SPEC.md`, verify with `NexusMind/scripts/verify_block_ledger.py`. |
