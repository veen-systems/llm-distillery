...
| **cultural-discovery** | v6 | (v5's) | **CUTOVER MERGED 2026-08-13, serves from the first cycle after 16:04.** v5's student verbatim (adapter md5 `bd4e79f2…`, byte-identical), so **scores do not shift** — what changes is that v6 ADDS Stage-1 e5 probe screening v5 never had: **63.7% of the firehose becomes a probe estimate, not a model score** (`stage_used: stage1_low`). ⚠️ `stage_used` is **dropped at the ovr handoff** (NM#351), so no consumer can tell the two apart. Remaining: refit `normalization.json` from real 6.0 rows (currently n=3,680 from 5.0). Detail: `memory/cd-v6-probe-hypotheses.md` |
