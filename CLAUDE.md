...
*Last updated: 2026-08-08 (afternoon) — **NM#300 and LD#88 VERIFIED on production rows and CLOSED**: `content_length` and `stage_used` are **100% populated in all six filters** (17:10 cycle), from 0 of 50,605. It was **FIVE allowlists in series, not two** — the morning's fix corrected hops 1 and 5, and the next cycle still read **0 of 2,170 with both fixes provably loaded**. The three missed hops all sat on the *response → result-object* boundary; the earlier "verified there is no third" had checked the *article → disk* seam, which is true and is not where the loss was. **Patching the sender proves nothing unless the receiver's parser is checked too.** Unasked bonus, now that `stage_used` reaches disk: **no surfacing article is ever probe-scored** (`stage1_low` peaks at raw 0.75–1.50 against op-points of 2.25/4.0; `surfacing AND stage1_low` = **0** everywhere) — the hybrid design's core safety claim, assumed since 2026-02 and never measured. **I also took the pipeline down**: I decided to KEEP `.bak` files as a rollback for an unverified fix, and one of them sat under a deploy-gated path, so `nexusmind.service` failed at 16:07 and would have failed every 4h; the commits were already pushed, so git *was* the rollback. Recovered 16:23, no data lost. **cd v6 is unblocked** (Hub repo created, normalization fitted n=3,680, 9/9, loaded end-to-end) but **deliberately NOT deployed**. **LD#93 step 4: do not set the cap** — a cap ≥ the op-point removes zero false positives, and my residual was understated 3× before the FluxusSource session corrected it. Shipped: **NM#303** (Contract A had never met production either → **NM#304**, 4 defects), census check-A hardening, the `commit-msg` false-fail fix, framework → **v1.17.0**, **agent-ready-projects#33**. Board **197**.*
