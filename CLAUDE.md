...
| **solutions** (renamed from sustainability_technology, ADR-012, 2026-07-18; pkg `filters/solutions/v4`, field `solutions_analysis`) | v4 | **TRAINED + CALIBRATED + GATED (2026-07-21). Deploy decision pending.** Gemma-3-1B+LoRA, 3 epochs, val MAE 0.564. `calibration.json` fit (marginal, +0.3%; `score_scale_factor 1.6093`). Step-8 runtime scorer written (`base_scorer.py`/`inference.py`, was missing → calibration first failed on it). **ADR-021 gate** (1,500 unscreened holdout, op 3.0): recall **0.45** / prec **0.78** / spec 0.99 / F1 0.57 / Spearman 0.46 — precision-strong, **recall-weak**. Op-point sweep: best F1 ~2.25 (recall 0.56). Gatekeeper cap 3.0≡2.9 (inert → keep 3.0). **Recall ceiling ~0.58 is STRUCTURAL** (52/61 misses scored <2.5 = clear disagreement, incl. 13 high-band): training corpus was e5-*screened* → model learned the screenable manifold, misses unscreened production solutions = the documented access-bias limitation → v2 (source expansion + AL), not an op-point/retrain fix. **NEXT (next session): compare gate metrics to other filters → op-point decision (2.25 vs 3.0 vs hold-for-v2) → deploy (inference_hub.py, normalization from prod-base-rate rescore, Hub, NexusMind wire-in, retire foresight).** Model gitignored → gpu-server + local backup. See `filters/solutions/v4/README.md` + `DATA_SETUP_PLAN.md`. NB: v4 replaces both sustech-v3 and foresight-v1 scorers | ovr.news Solutions tab |
