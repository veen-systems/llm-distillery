...
*Last updated: 2026-08-12 (afternoon). **Framework pinned v1.23.0; upstream is v1.25.0** — adoption deferred to the next session by the owner, pre-triaged in `docs/TODO.md`. **#109 arm A is CLOSED: WITHIN NOISE, which closes #105's `cultural_discovery` half** — a retrain there is a base-rate change, not a label repair. **NM#310 answered: a compute story** (median +0.112, 3.6% of crossings, all upward). **Three noise floors now exist and must not be inherited** — batch 0.16, cross-box 0.16, oracle decoder 0.436/0.687; see `memory/score-batch-shape-noise.md`. **#106 still awaiting the owner's one-line close**; *should ovr.news enrich* deferred behind ovr#312. **Next actions and full decision state: `docs/TODO.md` top block.** Session records: `memory/project_session_2026_08_12_afternoon.md` and the files it links.*
