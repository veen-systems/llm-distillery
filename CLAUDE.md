...
framework_reconciliation: |
  ...
  Re-measured here after the fixes: 25 pass, 0 fail, 0 error, 0 malformed,
  6 manual, exit 0 — all 38 annotations accounted for (7 are prose mentions
  inside code spans, 6 of them in `memory/gotcha-log.md`). One annotation needs
  `VERIFY_TIMEOUT=120`; the default 30s reports it ERROR and it is NOT broken.
  v1.22.0's `review-changes` negative-check rule ADOPTED into the project-local
  skill. Its `templates/project-file.md` 'Active work' section is N/A by the
  template's own instruction (delete it where the tool has auto-memory).
  v1.23.0's `audit-context` placeholder markers: skill is current, but 0 paths
  are marked here — DEFERRED to the next `audit-context` run, because a marker
  is only meaningful against a live findings list.
  All three user-global skills verified byte-identical to the v1.23.0 TAG.
...
- **Before using any source as evidence, establish what it EXCLUDES.** *(7th
  occurrence.)* Applies to data, to nested structures, to prior work, to literature,
  to hosts, and to **time**. A wrong path and a dead field both read as zero, and the
  wrong one is the more exciting finding. If it is a denominator, a baseline, or a
  claim of absence — enumerate the source first.
...
| **cultural-discovery** | v6 | (v5's) | **CUTOVER FAILED AND WAS REVERTED 2026-08-13** — stage 2 loaded from the Hub, which cannot work on gpu-server (`HF_HUB_OFFLINE`); scorer returned 500, smoke test caught it, one cycle lost, no bad data. Fixed + verified offline (`dcf2860`), **not redeployed**. ⚠️ **v5 ALREADY runs two-stage** — `filter_loader.py:148` sets `hybrid_class` from the PRESENCE of `inference_hybrid.py`, not from `config.yaml` (v5 declares no `hybrid_inference` block and screens anyway: **54.9% `stage1_low` measured**). So v6 does NOT introduce probe screening; it changes the probe/threshold, ~63.7% vs 54.9%. Detail: `memory/cd-v6-probe-hypotheses.md` |
...
| **Anything about the pipeline CONTRACTS — schemas, validators, what a row carries between repos** | **`docs/decisions/2026-08-14-contract-a-envelope.md`** (the envelope, **LANDED** — Contract A **1.20.0** on NexusMind `main`) then **`docs/CONTRACTS_PLAN.md` § *Round 3*** + **`memory/stamp-contract-integrity.md` § *The contracts layer***. ⛔ **Do NOT quote "four validators"** — a behaviour sweep found **21+**, and counting them was the wrong question. The finding in its final form: **exactly TWO shape checks are both automatically invoked AND looking at real production bytes**, and between them they assert eight key names and two strings' max length. ⚠️ Read the traps before quoting a number: the validator counts **errors, not rows**; `format` is declared and never asserted; don't grep bare field names; a `grep -rIl` that prints nothing is a **broken verify command**. |
...
| **Reading a number off NexusMind production data** | **`memory/nexusmind-data-sources.md`** — `filtered_*.jsonl` ALSO drops source-type-excluded rows (scored, then discarded — worth 0.129 on investment_risk), and `data/raw/` is pre-enrichment. Reconcile denominators before diffing two sources. ⛔ **`live_articles` is NOT the reader population** *(corrected 2026-08-16 — this line asserted it was)*: the view is legacy and off the build path, and the deployed view has drifted from its own source with **no code path that can reconverge them**. Use `getArticlesForBuild`, or the `articles` table as a superset. `weighted_average` there is NORMALIZED, not raw. |
| **Quoting any Google News number, or touching the GN population** | **`memory/google-news-corpus-hypotheses.md`** — GN is 25.7% of the corpus and **100.0% sub-300-char headline echoes** that **NexusMind's `pre_enrich` cannot enrich** — 35,229 attempts, 0 replacements, because NexusMind has no GN resolver (NM#310). ⚠️ **Always name the fetcher: ovr.news DOES resolve these URLs** via Google's `batchexecute` and enriched 74 of 103, so "GN can never be enriched" is refuted as a general claim. Training corpora are 0–4.9% GN. Five claims are already REFUTED there, four of them denominator errors. **Never oracle-re-score a GN row** (median 89 chars, below the floor that exists for exactly that reason), and never match GN on a `gn_` key prefix. It is a strict subset that undercounts by **5.12× on CONFIGURED FEEDS** (302 vs 59) but only **1.64× on EMITTED ROWS** (25,607 vs 15,655) — the 59 prefix feeds are the broad, high-volume country proxies. **So a row-share sanity check reads as roughly-right while the feed-level population is off five-fold.** Always name which quantity; an *item* count is a third number again (2.18× on 2026-08-08, 1.64× over 08-10→08-14) and is unstable by construction, because it moves with which feeds were due. |
