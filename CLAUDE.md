...
*Last updated: 2026-08-09 (night) — **sent to start #102; did its prerequisite instead.** Cross-box parity is now MEASURED for the Gemma student: b650 and gpu-server's serving venv agree **completely at the 4.0 op-point** (0 verdict flips of 660, identical confusion matrix — so `uplifting v7`'s spec **0.9189** is production's number and #102's premise stands) and **disagree at 4.5** (3 flips; spec 0.9730 vs 0.9662). **A box is cleared at a threshold, never in general** — the e5 probe's 4.2e-6 does not transfer, max calibrated |Δ| is **0.2008**, above the #95 floor. Sweep on production predictions: 4.5 puts uplifting's FPR at **2.70%** (from 8.11%), between solutions and nature_recovery — right direction under ADR-023, **not yet a decision**. **Two corrections to my own prior claims**: b650's GPU failure is a missing `python3.12-dev`, not a libcuda link error; and the 660-row run takes ~16 min (b650) / ~30 min (gpu-server CPU), not ~7. Production is frozen in `constraints/production-gpu-server.txt` — five interpreters across four boxes all satisfied `requirements.txt`'s ranges differently. **NOTHING DEPLOYED. Full record: `docs/TODO.md` top block, `memory/project_session_2026_08_09_night.md`, `docs/evidence/2026-08-09-cross-box-parity-uplifting-v7.md`. NEXT: #102 step 2 (needs no re-scoring — production predictions are committed), then the 21 `solutions_story` adjudications.*
