...
*Last updated: 2026-08-07 (six owner decisions taken one at a time — two were **stale as listed**, already decided in a repo the board does not read; a **seventh is newly open**: ovr.news's own 1.3/1.5/1.7× corroboration boost, which NexusMind's `1bbadb5` does **not** reach because ovr.news never reads `display_rank`. Two review rounds found 5 code defects and ~20 doc errors, nearly all same-day: the working rule above is escalated to occurrence **7** because both new instances had **correct callers** and were caught only by executing the guard. Corroboration precision is **0.560**, not the cap-disabled 0.283 headline.)*
