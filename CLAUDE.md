...
*Last updated: 2026-08-08 — **LD#101 confirmed by outcome and closed** (`eval_aggregator` rows **21 → 0** in all six filters, with the control that the same cycle's input carried 22), but the check pre-registered for it **would have reported failure**: `source_filter excluded N` came in at **86** against a "must exceed 121" baseline, because that line aggregates all types and swings 69 → 121 → 86 in one day. **A metric that moves for reasons unrelated to your change cannot confirm your change** — and three of my instruments broke that day against zero broken conclusions (that baseline; a `len>3` language heuristic that misread `zh-cn`; a watcher whose hour-glob matched the date and fired instantly on a pre-fix file). The fix was also alive only as **uncommitted edits on one box**. **NM#300 was two allowlist drops in series** — the Pydantic `FilterScoreResult` kills it first, so fixing `main.py` alone would have looked like a failed fix. **And the contracts had never met a production row**: `tests/unit/test_contracts.py` validates fixtures, and the first run over 2,400 live rows gave **908 violations** — `image_confidence`, declared 0–1, is a **raw logit**; the producer was right and the contract had been wrong since it was written, which no fixture could have caught. New: `scripts/stamp_census.py` (population + consumers), which **failed its own acceptance test first**. Board **196**; **Chain 8 closed by a parallel session** 30 minutes before I commented asking for a decision already taken, so **no calendar-bound item remains**.*

*Prior context (2026-08-07, night) — the "present, configured, unreachable" rule above reaches occurrence **8**, and this one is reader-visible: FluxusSource stamps `type_classification: eval_aggregator` on the three FS#120 evaluation arms, **nothing reads it** (zero hits in NexusMind `src/`, this repo's `filters/`, ovr.news `src/`; no filter's `excluded_source_types` lists it), so an A/B measurement rig is **scored by every filter and published** — 28 rows in `ovr.news/data/ovr.db`, a floor, including a funeral/murder story at tier `high` (**LD#101**). It was found by a completeness review of my own evening's work, not by the work itself. Same night, a three-lens battery withdrew two of my published recommendations: `newsdata_eval` filters on **publisher location**, not topic, so my "77–97% off-topic / add `country_queries`" was the wrong test **and** a fix that would have destroyed the gate's geographic-vs-topical comparison (real figure: local-publisher share **40% / 12% / 8%**, which is probably H3's *answer*); and the FS#134 outage argument was withdrawn because a renamed venv, not `datasketch`, caused it. Board is **198 open**, not 195 — the previous pass never re-counted after filing its own two issues.*

*Prior context (2026-08-07, earlier): six owner decisions taken one at a time — two were **stale as listed**, already decided in a repo the board does not read; a **seventh is newly open**: ovr.news's own 1.3/1.5/1.7× corroboration boost, which NexusMind's `1bbadb5` does **not** reach because ovr.news never reads `display_rank`. Corroboration precision is **0.560**, not the cap-disabled 0.283 headline.*
