...
- **Before shipping any gate, cap, threshold, config key or stamp, name the caller that loads it — and then prove the outcome changed at the END of the run.** *(7th occurrence, three self-inflicted.)* **Naming the caller is no longer sufficient, and 2026-08-07 is why.** Two guards shipped that day had *correct callers on the right paths* and still did nothing: one was reverted by a later pipeline step re-sending the old value through a `COALESCE` merge (123 stored rows already carried the signature), and the second was a **complete no-op** because a *different* commit point short-circuited before it — while its own comment asserted it was "the only point every source has in common". Both passed unit tests on the predicate. What caught them was an adversarial lens **executing the guard and printing the resulting state**. So: if a guard's whole value is that it changes an outcome, run it and show the outcome; a green test on the predicate proves only the predicate. And ask "is this the only writer of this field?" — not just "is my code reached?". Two related traps from the same day: **a comment explaining *why* code is safe is a claim like any other** (two of mine were false and would have been trusted), and **if a criterion depends on "now", encode the criterion, never its answer** — a hardcoded `aged: true` derived from a date was stale the moment it was written and left a reader-visible defect behind. A mechanism that is present, configured and unreachable is this repo's defining failure: ducroq/NexusMind#284 (per-filter prefilters never ran, six months), #94 (a gatekeeper binding 0 times in 191,616 articles), ducroq/NexusMind#281 (a gate that could never fire), ducroq/NexusMind#300 (the #93 `content_length` stamp computed then dropped — 0 of 50,605 rows), and `filters/cultural_discovery/v6` (a `hybrid_inference` block and probe shipped into a package with no inference module — **written the same day the other four were documented**). That last one is the point: knowing this failure mode does not prevent it; only running the check against your own work does. **Never infer runtime behaviour from the presence of a config key.** Two smells that must trigger the check: a package that passes self-tests but has never been loaded end-to-end, and a field initialised to `None` and populated somewhere you have not read. **Third smell, added 2026-08-09: a feature or check validated on "production data" without naming WHICH STAGE.** The arXiv `Announce Type:` prefix is a 91.6%-reliable detector on the collection corpus and reads **0.000** on NexusMind rows, because enrichment re-fetches the body in between. Both are production data. A detector built on the second would have been correct, reached, and inert — this failure mode again, with the call path perfectly fine.
...
*Last updated: 2026-08-09 — **corroboration: the change I was sent to ship was already refuted, and nothing shipped.** My brief said the 340-pair precision panel was UNADJUDICATED; it was adjudicated 2026-08-06 and the verdicts live in a **gitignored NexusMind dir never copied to b650** — *enumeration-is-not-inventory*, applied to a machine. NexusMind's V&V registry already recorded `title_body@0.96/0.92` as **DOES NOT SHIP** (it rejects 70% genuinely-same-story pairs at its own margin). Added the missing piece — the turn-over point, on a fresh 680-pair judged panel with a rule pre-registered before the run: **`title_body@0.94/0.90`** passes all three gates but **only TIES live under the #95 standard** (overlapping intervals → not distinguishable), because the live baseline itself moved **0.283 → 0.173** between draws at n_eff 36. **My own D2 was weaker than this project's own rule.** `title_body@0.92/0.88` is decisively dead: **17.7M merged pairs**, giant stratum **0/25**. **The biggest lever turned out to be a GATE, not a similarity feature** — primary scientific documents are **20.6–30.4% of everything merged and 0–14% correct**, and excluding them beats any threshold move (0.344 → **0.459**). Traced to root and filed: **FS#143** (arXiv category feeds prefix the body with `arXiv:… Announce Type:`, shifting `content[:500]` so the collection hash can never collide — **600 duplicate papers per 48 cycles**, all same id AND version; the owner's fix beats mine: **drop the plain `arxiv` feed**, removing 100% of the class for 77 papers/8 days), **FS#144** (**308 Google News feeds collide on domain `google.com`**; 86% of `academic` rows are Global South news — and the root defect, *topic read as source type*, survives retiring Google News: Smithsonian/ScienceAlert/STAT have ONE category each), **NM#305** (detect primary literature from **article features**: union of academic-API metadata / `[Clinical Trial]` prefix / DOI gives **1.000 recall and 0.000 on Guardian, Ars, Smithsonian, STAT, ScienceAlert**; its apparent FPs were CrossRef/OpenAlex/Nature — literature the source list had missed). **Schema answer: Contract A is `additionalProperties: false`, so stamping at collection REQUIRES a schema change**; Contract B is permissive and would accept it silently, invisible to the census. **STAGE TRAP**: the arXiv prefix is 91.6% reliable at collection and **0.000 after enrichment** — measure a feature at the stage it will run, or ship a dead detector (#284/#300 shape). Features probed on 599 adjudicated pairs: **shared numbers REFUTED** (AUC 0.503–0.581, despite a good argument), **time CONFIRMED at 0.809**, independently replicating INST-10's 0.798 — built, and **still `temporal.enabled: false`**. Five of my own errors caught by checking, including a **91% cross-language figure that does not replicate** (2/4 on the new panel). **Nothing deployed; no production config changed in any repo.** Regression check green: `content_length`/`stage_used` **100% on the latest cycle**.*
