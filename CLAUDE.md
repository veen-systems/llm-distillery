...
*Last updated: 2026-08-09 (evening) — **sent to fix a calibration defect; there wasn't one.** The 173× surfacing spread is mostly **genuine base-rate difference**: 160 production articles oracle-graded put `nature_recovery` at **87% precision** with only ~8 true positives per 2,190 articles, and bands below student-1.0 (96.7% of the corpus) returned **0 of 50**. **Two of my own headlines retracted the same day** — an isotonic-crush explanation (real, ~3 articles/cycle, not the lever) and a six-filter MAE ranking (invalid: six populations, one number). The owner's correction became **ADR-023**, now a Hard Constraint above. **One irreversible risk closed**: `uplifting v7`'s adapter existed on a single disk; three production probe pickles existed only on gpu-server. `uplifting v7` has its **first accuracy record ever**. **NOTHING DEPLOYED and nothing needs to be.** Framework v1.18.0, no drift. **Full record: `docs/TODO.md` top block and `memory/project_session_2026_08_09_evening.md`. NEXT: #102** (uplifting specificity — 8.1% FPR vs 2–3%, run the ADR-021 gate on a 4.5 op-point), then adjudicate the 21 `solutions_story` candidates.*
