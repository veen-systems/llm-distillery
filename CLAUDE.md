...
| **solutions** | v5 | **In development — adding missing e5 probe (two-stage architecture).** v4 quality gate (2026-07-26) found 27% policy/regulation bleed because the 1B model was doing both screening AND scoring in a single pass. v5 adds Stage-1 e5-small embedding probe (ADR-006) following nature_recovery v4's pattern. `inference_hybrid.py` created, `train_probe.py` generalized for multi-filter use. Probe training pending: `scripts/train_probe.py --objective recall`. FILTER_PLAYBOOK.md §4: probe now **REQUIRED for needle filters.** | ovr.news Solutions tab |
