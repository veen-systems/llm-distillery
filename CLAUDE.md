...
| **investment-risk** | v6 | recall 0.72 / spec 0.97 | **PAUSED 2026-08-25** (owner: Aegis is dormant). Never an ovr.news lens — its only consumer was the Aegis export, now off too. **PAUSED ≠ REMOVED**: package, HF Hub repo, Contract C and 251 days of archives all stay; un-pause = restore `pipeline.enabled_filters`, `pipeline.aegis_export.enabled` **and the `deploy/smoke_test_articles.jsonl` row** — ⛔ **it is THREE files, not two**: the missing fixture row failed the deploy gate closed and cost the 2026-08-25 20:03 cycle (now a unit test, `tests/unit/test_filter_integrity.py`). Was op-point 4.25. Decision: `docs/decisions/2026-08-25-pause-investment-risk.md` |
...
| **Asking what an article field IS, or where a blocked article went** | **`NexusMind/contracts/article-record.schema.json`** — the record's definition since 2026-08-24, owner-ruled **prescriptive** and **composed** (`allOf: [$ref Contract A, nexusmind_layer]`), so producer fields have one definition and cannot drift. Human half: `NexusMind/docs/ARTICLE_RECORD.md`. **The per-field answer is `NexusMind/docs/ARTICLE_RECORD_REGISTER.md`** — generated, 109 owned fields, population + semantics + writer + migration target, and it exits 1 when a field is observed and classified nowhere. ⛔ **Do not quote a field count from any of them** — every count is a WINDOW (260 at 12 cycles, 229 at 2); print the window or say nothing. ⛔ **"The layer is declared nowhere" is FALSE** — the undeclared set is **19** by exact-path counting (`scores.<dims>`: the container is declared, the dimension names are not). ⚠️ The census's three mis-defined columns were **fixed 2026-08-24** and its two blind ghosts **2026-08-25**; `pres%` ≠ `fill%`, and a `<obj:N>` row is a presence marker, not a value. Blocked articles: `docs/BLOCK_LEDGER_SPEC.md`, verify with `NexusMind/scripts/verify_block_ledger.py`. |
...
| **Quoting any #121 number, or building an opinion/editorial genre stamp** | **`memory/opinion-genre-hypotheses.md`** — #121 (does opinion/editorial framing inflate lens scores?), measured over 233,338 rows: the **within-source control dissolves the effect in 5 of 6 lenses** (`belonging` 5.9× → MH OR 0.98). Only `investment_risk` survives (OR 2.05), and a **topic confound survives inside each source**. ⛔ **#121's issue body scores `solutions` at op-point 4.0; it is 2.25** — that manufactured one of the two zeros its own conclusion rested on. ⚠️ `nature_recovery`'s null is uninformative (68 of 92 strata tied 0 vs 0). |
