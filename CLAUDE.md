...
*Last updated: 2026-08-12. **Framework current at v1.23.0** (v1.22.0 + v1.23.0 adopted 2026-08-12; last session's "unreleased candidate" shipped). **`solutions v6`'s `community_practice_strength` is NOT dead, just rare** — see `memory/solutions-v6-dimension-hypotheses.md` before touching any dimension weight. **Google News mass in the normalization CDF is measured and small** (max 0.367, all gate crossings downward) — `docs/evidence/2026-08-12-gn-share-of-normalization-cdf.md`; do not re-derive it. Owner rulings 2026-08-12: **#107 CLOSED** (positive outcome suffices), **#109 Arm A approved / Arm B held**, **#106 awaiting the one-line close**, and *should ovr.news enrich* still deferred behind ovr#312. **Next actions and full decision state: `docs/TODO.md` top block.** Session records: `memory/project_session_2026_08_12.md` (+ `_2026_08_11_evening`, `_afternoon`).*
