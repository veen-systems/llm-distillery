...
| **cultural-discovery** | v6 | (v5's) | **NOT LIVE, not blocked.** Package parity 2026-08-06 (#98): v5's student + e5 probe + commerce-only prefilter, **no retrain**. Hub repo private, adapter md5-identical to v5; `normalization.json` fitted n=3,680 from `filter_version=5.0` rows; `--check-hub` 9/9; loaded and scored end-to-end. **Remaining: the cutover deploy, then refit normalization from real 6.0 rows.** Detail: `memory/filter-status.md`, `memory/cd-v6-probe-hypotheses.md` |
...
*Last updated: 2026-08-12 (evening). **Framework pinned v1.23.0; upstream is v1.25.0** (`889b038`, re-checked at close) — adoption is the **first task of the next session** by owner instruction, pre-triaged in `docs/TODO.md`. **#106 is CLOSED** (residue ≤0.128 normalized, every enrichment-gate crossing downward) — ⚠️ and **ADR-007 retires Google News population A ONLY**, so "GN gone" is neither a decided outcome nor a fix for short-content scoring. **#109 arm A CLOSED: WITHIN NOISE**, closing #105's `cultural_discovery` half. **NM#310 answered twice over** — a compute story (median +0.112, all crossings upward), and now **NexusMind's `pre_enrich` attempted 35,229 GN rows and replaced ZERO**; state it as mechanism + measurement **and always name the fetcher**. ⚠️ The stronger form ("a property of the URL scheme, no fetcher change moves it") is **REFUTED 2026-08-12** — ovr.news resolves GN via `batchexecute` and enriched **74 of 103**; the over-generalization had already propagated into ovr#312 as a premise and licensed a "don't fix the resolver" call that would have cost 22 of 38 published articles. Scoped version: `memory/google-news-corpus-hypotheses.md`. **Three noise floors exist and must not be inherited** — batch 0.16, cross-box 0.16, oracle decoder 0.436/0.687; see `memory/score-batch-shape-noise.md`. *Should ovr.news enrich* is still deferred behind ovr#312. **Next actions and full decision state: `docs/TODO.md` top block.** Session records: `memory/project_session_2026_08_12_evening.md` and the files it links.*
