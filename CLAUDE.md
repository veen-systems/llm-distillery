...
| **Anything about the pipeline CONTRACTS — schemas, validators, what a row carries between repos** | **`docs/CONTRACTS_PLAN.md`** (the five-repo plan, NOT approved and NOT executed) + **`memory/stamp-contract-integrity.md` § *The contracts layer*** (measurements, five instrument traps, three rules). **The estate has FOUR contract validators and none watches the failure.** ⚠️ Read the traps before quoting a number: the validator counts **errors, not rows**; `format` is declared and never asserted; don't grep bare field names; a `grep -rIl` that prints nothing is a **broken verify command**. |
