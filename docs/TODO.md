...
- [ ] **#95 — pin `batch_size` in the production scorer. NOW THE SOLE BLOCKER on #93 step 4** (promoted 2026-08-05: #92's identification cleared the other one). A score is not a function of the article alone: batch composition moves it up to 0.162 and flips 7-9% of near-boundary tier/visibility decisions (measured on solutions v6 @2.252 and uplifting v7 @4.0). Pinning does not remove the noise but makes a cycle reproducible. Then record the noise floor in the ground-truth gate and `docs/NORMALIZATION_METHOD.md`.
