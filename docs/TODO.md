...
- [ ] **NM#285 — the shadow measures a truncated article** (`title`+`content` only; `url`/`source`/`description` never reach the scorer). Four filters cluster at ~0.59 by artifact; **cd's 0.255 survives** because its dominant rule is content-based, investment_risk's does not. **Gates every per-filter enforcement decision.** Recommendation on the issue: run prefilters **pipeline-side** as a preprocessing stage (Option C) rather than widening the scorer API — a prefilter's purpose is to avoid the expensive call, and running it inside the scorer captures almost none of the saving. Measure the per-filter truncation effect first.
- [ ] **NM#286 — ADR-022 gaps**: commerce has no `enforce` key (rollback needs a code change); a consumer-side commerce drop in `enrich_survivors.py` (harmless only until the first is fixed — they must move together); violence stamping skipped in three run modes (single-filter, `--no-dedup`, dedup exception).
