...
- [x] **uplifting v7 normalization refit** (NM#279) — **EXECUTED 2026-07-31** (owner go): 18,130 MEDIUM+ articles, anchored at 4.0, raw 5.0 → norm 5.17 (was ~3.0 under the ×1.1976 unit-mismatch fit); raw_max 7.86 < ceiling 8.35 confirms artifact gone. llm-distillery `bbe1d78`, NexusMind `0c35731`, byte-verified on sadalsuud + gpu-server. *Verify live next cycle (§6 step 4).*
- [x] **belonging v1 normalization refit** (NM#279 / #74) — **EXECUTED 2026-07-31**: 4,827 articles, anchored at 4.0. Same commits/deploy. *Verify live next cycle; then close #74 as "explained".*
- [x] **NexusMind `_assign_tier` double-cut (NM#280)** — **IMPLEMENTED + DEPLOYED 2026-07-31** (NexusMind `73c1ec3`, after refits went live in the 08:12 cycle): visibility = raw ≥ op-point; normalized grades passers; caps keep suppression power; rollback `NM_TIER_RAW_VISIBILITY_GATE=0`. *Verify next cycle: count(tier!=low)==count(raw≥op-point) per filter, nr +27–37% visible.*
- [x] **cd v5 dead prefilter (#86)** — **FIXED + DEPLOYED 2026-07-31** (`cfafde2` / NexusMind `f0d6cc6`): multilingual topic gate, measured 24.5% pass on 14,923 production rows, 0.23% model-visible collateral (mostly off-lens science). 13/13 self-tests. *Verify next cycle: blocked>0, pass≈0.25 → then close #86.*
