...
- [x] **LD#92 corrected at n=60/group** (660 articles, deepseek-chat, 0 errors). **uplifting does not replicate** — DiD +0.44 but **not significant** (permutation p=0.054; fails Holm across 6 filters), so read it as *no detectable length effect*. solutions v6 shows DiD −1.13 [−1.74, −0.52], MAE 1.51× — but **NOT IDENTIFIED**: the design selects on the student's own score at each op-point, and under differential noise that artifact alone reaches −0.82 to −1.61. **Do not fit a solutions cap yet** — re-run at a second op-point first (an artifact moves with the threshold, a real effect does not). The "P = 0.0000" bootstrap is **withdrawn** (sampled without replacement; true value 8.0e-5; and it asked the wrong question). Root cause looks like an op-point mix-up — LD#92 used **2.25**, which is *solutions'* op-point; uplifting's is **4.0**. The "924 / 15.0%" figure reproduces exactly at 2.25 and collapses to **117 / 1.9%** at 4.0. "~460 bad articles per 8 cycles" does not survive.
...
- [ ] **Fit the solutions short-content cap** (#93 step 4) — blocked on #92's second-op-point re-run. The mechanism is in place; the value is not justified. Do not set `short_content.cap` on any filter until an artifact-free estimate exists. **When running that test, pin `batch_size`** — #95 shows op-point membership moves on its own.
- [ ] **#95 — pin `batch_size` in the production scorer.** A score is not a function of the article alone: batch composition moves it up to 0.162 and flips 7-9% of near-boundary tier/visibility decisions (measured on solutions v6 @2.252 and uplifting v7 @4.0). Pinning does not remove the noise but makes a cycle reproducible. Then record the noise floor in the ground-truth gate and `docs/NORMALIZATION_METHOD.md`.
...
- [ ] **Re-run the NM#284 shadow** now that the length floor is out of the prefilters — its pass rates finally describe lens behaviour, which is what LD#90 item 2 needs. Rates measured before 2026-08-03 are not comparable to ones measured after.
