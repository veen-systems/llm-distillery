...
- [ ] **cd v5 dead prefilter (#86)** — fix is correct but **INERT**. Verified 2026-08-01: production still stamps 2647/2647 pass. Replaying the gate over those same rows gives 28.8% pass (1799 `no_cultural_topic_signal`) — the gate works, it is never called. **Root cause is not cd-specific: no filter's prefilter has ever run in production.** `deploy/gpu-server/main.py` sets `use_prefilter=False` (L915) + `skip_prefilter=True` (L1318) since `66582e7` (2026-02-10); all eight deployed filters stamp 0 blocks. Filed **NM#284**. #86 stays open blocked on it.
- [ ] **NM#284 — re-enable per-filter prefilters** (NEW, 2026-08-01, blocks #86 and gates #87): stamp `passed_prefilter` truthfully always (ADR-022), enforce via each filter's `config.yaml prefilter.enabled` with an `NM_FILTER_PREFILTER_ENFORCE=0` lever, shadow first, then flip per filter. Adds the declared-vs-observed pass-rate drift assertion. Op-point / normalization re-derivation for affected filters is downstream of the flip.
