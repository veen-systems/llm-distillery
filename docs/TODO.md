...
- [x] **Universal obituary detector (#51)** — v3 shadow-deployed NM#185 (2026-07-27), v4 corrective retrain complete (2026-07-28). Op-point evidence 2026-07-30 (#83): v4 promoted at **0.90** not 0.95 (owner-flagged Farouq obituary scores 0.937 on v4; heldout FP set identical at both thresholds). NexusMind swap `02da4fe` auto-deploys next cycle, still shadow. 2026-07-30 session 2: v4@0.90 shadow deployed+verified (Farouq 0.9372 via endpoint). **FN-delta gate FAILED** (21/28 gap articles are real obituaries, 5 unrecoverable by threshold) → v5 corrective retrain (21 panel-graded hard positives in rollup_fn_delta_fp5_2026-07-30.json), enforcement sign-off BLOCKED; interim shadow (v4@0.90 vs v3@0.95) = owner decision on #83. Then enforce → ovr#204.
