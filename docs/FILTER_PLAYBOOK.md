...
### 1b. Before labelling — the QUALITY check (quality over quantity)
*Added 2026-09-24 from `human_thriving v8`: labelled for $6.89 (EXP-010), trained, deployed, and only THEN found that ~1 in 5 live passers fall outside the documented definition (EXP-029). Every item below would have been cheaper before the labelling run than after it.*
- **Success was measured only on the labelling corpus → the reader-facing defects appeared after deploy.** Before the full labelling run, oracle-score a **production** sample (~100–200 recent articles, ~$1) with the candidate prompt and **read every passer** against a written list of what must never surface on this lens. Fix the prompt until that list is clean. ⛔ **Volume is never a target — quality is** (owner, 2026-09-24; ADR-023): judge the passers, never the pass rate. A thin tab is fine; junk on it is not.
- **Labelled with a prompt whose gaps were already written down → fixing them now costs a relabel + retrain.** v8's `STATUS.md` listed three owed gaps at deploy (commencement, money-worded proposals, the dropped court-relief clause #143); they were **12 of 62** live passers (EXP-029, p=0.0034). **Every known gap in STATUS/TODO is closed, or explicitly accepted by the owner, before the labelling run** — the student copies its oracle faithfully (0.968), so an oracle gap ships.
- **A hard gate inside the oracle prompt → the student leaks across it.** v8's prompt forced all six dimensions to 0–2 when harm is the subject; a regression head cannot learn a discontinuity and scored three such articles 4.66–4.85, just over the 4.50 op-point (EXP-031). A yes/no concern belongs **outside** the learned score — a separate detector stamp (ADR-022) or a gatekeeper applied in scoring code (`filters/common/filter_base_scorer.py`, as `nature_recovery v4` does) — or as a graded per-dim penalty (§1). If a gate must stay in the prompt, check the student's scores on gated rows near the op-point before deploy.
...
- **There is a measured noise floor under every score comparison: `|Δ| ≤ 0.16`.** A score is **not a function of the article alone** — it also depends on which batch the article was scored in. Same model, same weights, same GPU, same process; only batch composition differs (#95, measured 2026-08-03 on real Gemma-3-1B + LoRA, 120 production articles). Consequences, in order of how easily they bite:
  ...
  - ⛔ **Its sibling is NOT "cross-box", and this bullet said so wrongly from 2026-08-03 to 2026-08-29.** The decomposition (`docs/evidence/2026-08-10-b650-gpu-production-stack-parity.md`, uplifting v7, same 660-row split, four runs, one variable at a time) puts the **host term at exactly 0.0000 — 660/660 bit-identical across two different physical machines** once the library pins and the device match. The two real terms are the **library stack** (max |Δ| **0.2008**, 3 verdict flips at 4.5) and the **device, CPU→CUDA** (max |Δ| **0.1956**, 1 flip at 4.0, 3 at 4.5) — **both ABOVE the 0.16 floor**, so "same magnitude" was wrong too. The rule that replaces it: **match the pins and the device and the box is free; change either and re-run the harness AT YOUR THRESHOLD.** ⚠️ Two caveats the first version of this bullet missed: the host term was measured with the device held at **CPU**, so **CUDA-to-CUDA across boxes is an extrapolation, never measured**; and the threshold flag is on **`scripts/verification/diff_box_parity.py`** (`--threshold` / `--alt-threshold`), not on `box_parity.py`, which only dumps scores and takes no threshold at all. ⚠️ It is the **same three articles** every time, and a row at 4.4870 — *closer* to the cut — never **flips** (it moves 0.0440), so this is three fragile articles, not "the rows nearest the threshold". Measured over four runs of one 660-row split of one filter; not established beyond that.
    ⭐⭐ **RE-MEASURED 2026-09-17 WHEN b650'S GPU WAS SWAPPED (3090 Ti → RTX 5090), AND IT
    OVERTURNS HOW THIS BULLET'S "ABOVE THE 0.16 FLOOR" READS.** With a **byte-identical CPU
    control** (660/660, five weeks and a hardware swap apart) fixing the GPU as the only free
    variable: CPU→CUDA on the new card is **0.1572 with ZERO rows above 0.16 — and it still
    flipped 2 verdicts at 4.5**, the flipping rows moving **0.0467** and **0.1421**. ⛔ **So a
    term being under the floor does NOT mean it is free, and the converse framing above must not
    be read that way either.** A flip is a *small* delta sitting *near the bar*; a max-|Δ|
    comparison against a floor is structurally blind to it. **Read the flip count at your
    threshold — the magnitude answers a different question.** Third term from the same run: the
    two GPUs disagree with **each other** at **0.2357**, so ⛔ **stored b650-CUDA dumps are
    Ampere artefacts and must be re-dumped, never diffed**, and a device term is not even stable
    across one vendor's generations. The CPU-side host (0.0000) and stack (0.2008) terms are
    untouched — both held the device at CPU. ⚠️ Inference only; training across the swap is
    unmeasured (`H-DEV-1`). `docs/evidence/2026-09-17-b650-gpu-swap-parity/`, `EXP-038`.
...
6b. **Keep a local copy of the weights** in `filters/{name}/v{N}/model/` on situla. *(→ 2026-08-13)* **ENFORCED**: `preflight_deploy_guards.py` guard E aborts the deploy without one. Not repo hygiene — `filters/**/model/` is gitignored, restic walks the filesystem, so that tree **is** the off-site backup path. Three of six live filters had no such copy until 2026-08-13; their only homes were gpu-server (employer hardware) and a single-account private Hub repo. The four that were covered were covered **by accident**, not by decision. ⚠️ `git status` and a repo grep both report the weights absent whether they are there or not — `ls -l` on the path is the check that discriminates. Pull from the Hub (keeps the recovery chain Veen-owned), then delete the `.cache/` dir `hf_hub_download` leaves behind.
