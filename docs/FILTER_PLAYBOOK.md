...
- **Wrote a prefilter, assumed production used it → it never ran.** A filter's `prefilter.py` gates **oracle spend** in phases 3/5 and runs in llm-distillery's training path. It does **NOT** run in NexusMind's production scoring path: the gpu-server scorer builds every scorer with `use_prefilter=False` and calls `score_batch(skip_prefilter=True)`. True since 2026-02-10, found 2026-08-01 — all eight deployed filters stamped 0 blocks per cycle while every `config.yaml` declared `enabled: true`. NM#284 stage 1 now logs observed vs declared pass rate; enforcement is not yet on. **Don't size a filter's production volume on its prefilter's pass rate until NM#284 stage 3 lands.**
...
- **Quoted a prefilter's total block rate as lens signal.** Split base-class rules (`content_too_short`, `missing_content`) from lens rules first — the length floor blocks the *identical* articles for every filter and dominates the total (24.8 of 38.8 points across six filters). `memory/calibration-history.md` Dead Ends.
