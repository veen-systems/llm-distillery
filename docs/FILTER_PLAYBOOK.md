...
- **A matching pass rate is NOT a safety argument for enforcement.** `cultural_discovery v5` is the one filter whose observed rate matches its declared one (0.2605 vs 0.25) — and enforcing it would still block **15.5% of surfacing articles** (19.9% non-English vs 13.0% English, z≈2.6, p≈0.01). Rate agreement says the *volume* is right; it says nothing about *which* articles. **Before any enforcement flip, measure recall against articles the scorer already ranks above the op-point** (ADR-021 applied to gates, not just models). LD#86.
