...
- `filters/{name}/v{N}/normalization.json` — per-filter CDF lookup table (schema: `{"x": [...], "y": [...], "n_articles": int, "fitted_from": str, "fitted_at": str}`, where x = raw WA scores, y = normalized 0-10 scores; anchored fits carry one extra leading breakpoint and `stats.sample_min`)
