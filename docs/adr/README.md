...
- [ADR-012: Lens-Aligned Filter Naming](012-lens-aligned-filter-naming.md) - Rename filters to match ovr.news editorial lens names at next version bump. **Amended 2026-08-06 — the rename backlog is closed:** `cultural_discovery` + `nature_recovery` CANCELLED, `solutions` confirmed as-is, `uplifting` → `human_thriving` at v8. Rule: rename to the lens name only when it stands alone out of context (the Hub is a public standalone surface); otherwise keep the name or build `{qualifier}_{lens}`.
- [ADR-013: English Lens Names](013-english-lens-names.md) - All lens, filter and tab names use English; no Dutch (amends ADR-012). **Amended 2026-09-17: widened from names to all framework-internal text** (docs, ADRs, comments, memory, commit messages), ⛔ carving out match patterns, boilerplate strippers and test fixtures — data the code matches, not language it is written in. ⚠️ **Compliance is NOT established** and two violations are open; the rule is advisory, nothing enforces it
...
- [ADR-023: Asymmetric Loss — Precision over Recall](023-asymmetric-loss-precision-over-recall.md) - **We prioritise HIGH CERTAINTY over HIGH DETECTION**: be right about what is surfaced rather than catch everything. A false positive reaches a reader; a false negative is invisible and the slot refills. Optimise specificity/precision at the op-point; recall is a floor, not a target. **Never rank filters on MAE** — it is dominated by the ~97% negatives and depends on each split's positive rate. Compare on recall + specificity only. Active-learning batches sample ABOVE the gate. ⛔ **It binds REPORTING too** — every document publishing a recall figure states the priority beside it, because a low recall here is the decision working and does not read that way cold.
