...
- [ADR-012: Lens-Aligned Filter Naming](012-lens-aligned-filter-naming.md) - Rename filters to match ovr.news editorial lens names at next version bump. **Amended 2026-08-06: `cultural_discovery` and `nature_recovery` renames CANCELLED** — the Hub is a public standalone surface where `discovery-filter-vN` / `recovery-filter-vN` say less than the names they'd replace. Rename only when the lens name is at least as descriptive out of context.
