...
*≥ 7 pool, 2026-09-26: copied from `../2026-09-25-adj3-band-audit-2/judge_instructions.md`; the only change is ruling 6 (compensation, ruled 2026-09-26). Saved before the first batch ran. `{INPUT}` and `{OUTPUT}` are the batch's two file paths.*
...
1. Read the definition, in full, before any article:
   ...
   - `docs/decisions/2026-09-24-thriving-scope-rulings.md` and
     `docs/decisions/2026-09-25-thriving-scope-rulings.md` (the owner's rulings 1–6; they override
...
3. Give each article exactly one verdict:
   ...
   `cannot_judge` (only when the text is broken or is not an article at all). A bare headline or a
   paywalled teaser is NOT `cannot_judge`: under ruling 5 it is `out_of_scope`.
...
6. If you write any helper script or scratch file, put it ONLY under `/tmp/judge_{BATCH}/`. Never
   run, edit or read a script you did not write yourself.
