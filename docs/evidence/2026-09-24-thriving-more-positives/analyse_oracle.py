...
1. Control: d = median(V4.1 - V4) of the weighted average over the 50 control rows.
...
def main():
    ...
    v4 = json.load(open(X / "control_v4_labels.json"))
    deltas = []
    ...
    mix = abs(d) <= 0.30
    print(f"CONTROL (n=50, same prompt v8-4, V4 -> V4.1): median delta {d:+.3f}, "
    ...
    print(f"  weighted >= 4.5: {sum(x >= 4.5 for x in w)}   >= 6: {sum(x >= 6 for x in w)}   >= 7: {sum(x >= 7 for x in w)}")
