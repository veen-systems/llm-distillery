# Judge instructions — Nature recovery miss audit, RE-JUDGE under ruling NR-1 (given verbatim to every blind subagent)
...
4. Write `docs/evidence/2026-09-25-nature-recovery-miss-audit-2/out_{BATCH}.jsonl`: one JSON line per input article, **in input order**, with exactly these keys: `{"id": ..., "verdict": ..., "quote": ..., "reason": ...}`. `quote` is a short verbatim span from the article's own text that your verdict rests on (empty only for `cannot_judge`). `reason` is one sentence.
5. Reply with only the number of lines written and a count per verdict.
6. If you write any helper script or scratch file, put it ONLY under `/tmp/judge_nr2{BATCH}/`. Never run, edit or read a script you did not write yourself.
