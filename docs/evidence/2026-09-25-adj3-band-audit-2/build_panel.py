#!/usr/bin/env python3
"""Build the adj3 band-audit RE-RUN panel as PREREGISTRATION.md fixes it. Exit 3 on any plumbing mismatch.

Writes beside this file: input_A*.jsonl / input_B*.jsonl (blind) and bands.json (sub-band N).
The key (id -> sub-band) goes to datasets/audit/ht_2026-09-25/band2_key.jsonl, out of the judges'
...
SEED, BATCH, DRAW, B_SHARE = 20260927, 56, 40, 0.4
FIRST = REPO / "docs/evidence/2026-09-25-adj3-band-audit"
NEW_CONTROLS = {"balkan_klix_ba_80ffeaedf1e5": ("docs/evidence/2026-09-25-v8-adj3-live-audit", "out_of_scope"),
                "gn_africa_gn_tunisia_633f3342afd4": ("docs/evidence/2026-09-25-adj3-band-audit", "out_of_scope")}
BANDS = [("ge4.5", 4.5, 99.0, 826), ("b4.25", 4.25, 4.5, 182), ("b4.0", 4.0, 4.25, 197), ("b3.75", 3.75, 4.0, 245), ("b3.5", 3.5, 3.75, 313)]
...
first_ids = {json.loads(l)["id"] for l in open(FIRST / "band_key.jsonl")}
no_draw = first_ids | set(NEW_CONTROLS)
...
for name, lo, hi, n_expected in BANDS:
    ...
    panel += [(i, name) for i in sorted(rng.sample([i for i in pool if i not in no_draw], DRAW))]
n_ge45 = sum(a3[i] >= 4.5 for i in ids)
out = {"N_ge_4.5": n_ge45, "bands": bands, "cycles": 42}
if n_ge45 != bands["ge4.5"]["N"]:
    fatal("N_ge_4.5 mismatch")
...
for p in sorted(PILOT.glob("input_A*.jsonl")):
    for r in map(json.loads, open(p)):
        if r["id"] in ctrl_exp:
            controls[r["id"]] = r
for cid, (d, e) in NEW_CONTROLS.items():
    ctrl_exp[cid] = e
    for p in sorted((REPO / d).glob("input_A*.jsonl")):
        for r in map(json.loads, open(p)):
            if r["id"] == cid:
                controls[cid] = r
if len(controls) != 6:
    fatal(f"expected 6 controls, found {len(controls)}")
...
rng.shuffle(rows_a)
b_ids = set(rng.sample(sorted(want), round(B_SHARE * len(want))))
...
with open(D / "band2_key.jsonl", "w") as w:
    for i, b in panel:
...
print(f"panel {len(panel)} + 6 controls in pass A ({-(-len(rows_a) // BATCH)} batches); pass B {len(rows_b)} rows")
