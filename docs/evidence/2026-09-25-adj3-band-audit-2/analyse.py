#!/usr/bin/env python3
"""RE-RUN: junk share of everything adj3 would publish, per cut-off (PREREGISTRATION.md). Every stratum,
including >= 4.5, comes from this panel; the live audit is NOT used.

Reads bands.json, band2_key.jsonl, out_A*/out_B* beside this file (or --dir).
Exit 0 table printed, 1 a stop condition fired, 3 plumbing.
"""
# design-weights: READ. Every stratum is re-weighted by its N; the bootstrap
...
SEED, DRAWS, V8_POINT = 20260927, 10000, 0.338
...
# --- band panel
key = [json.loads(l) for l in open(args.dir / "band2_key.jsonl")]
...
if len(ctrl) != 6 or ok != 6:
    print("STOP: controls not 6/6")
    raise SystemExit(1)
...
for b in ["ge4.5", "b4.25", "b4.0", "b3.75", "b3.5"]:
    xs = band[b]
...
for cut, members in CUTS:
    ...
    vol = (N["ge4.5"] + sum(N[m] for m in members)) / bands["cycles"]
