...
⚠️ **A RESTATEMENT, ADDING NOTHING.** The three tests above are the whole of this step and are
repeated here in other words so that their order is unmistakable. **First**, name what the
article would still be about if its best sentence were deleted — that is the dominant subject.
**Second**, name the event that caused the article to be published today — that is the occasion,
and it is what the headline and the opening paragraphs report. **Third**, decide whether the
occasion is something completed and delivered to people, or whether it is a harm, a crime, a
death, an abuse, a worsening number or an institutional failure. Background does not displace
the occasion, and later paragraphs reporting real successes do not displace it either. Length
does not vote. A reader meets this article as its headline. None of this sentence introduces a
new rule, a new category or a new exception: it restates §1 exactly as written above, and every
decision is still made by the tests already given.
