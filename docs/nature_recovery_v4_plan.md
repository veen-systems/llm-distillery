...
- **C1. ✅ RESOLVED 2026-07-08 → chose (b): redefine `recovery_evidence` so documented pressure-removal / enacted enforcement (delivered protection) counts as ≥3.** Rationale: `content_type` never reaches student inference (`filter_base_scorer.py` has zero refs; student emits only the 6 dims), so option (a) would need a 7th head or a shared-scorer mutation (violates the "pure shared math" hard constraint). (b) is a prompt+relabel-only change, semantically honest (enacted pressure-removal *is* recovery-in-progress), keeps pledges/announcements low→capped, and moots §A.1 decoupling + reframes §A.5 (high protection↔recovery corr is now acceptable, not an abort). Original problem statement below for context:
...
**✅ §A.5 FALSIFICATION PILOT — PASSED (2026-07-08).** 28 stratified articles re-scored under the revised prompt (`datasets/scored/nr_v4_pilot_scored.jsonl`, $0.04). Delivered protection now surfaces via recovery_evidence path (b): Central Arctic fishing ban 1.8→5.15 (RE 4.0), São Tomé "declares 2 of 8" MPAs 3.5→4.75, French Polynesia 6.3→5.60, Europe dam removals ~6.7. Pledges stay ≤2.0; measured recovery unchanged high (5.4–7.2, no over-demotion); decline demoted (0.2–0.4, #60/#56 holds); conservation_appeal soft-penalty demotes borderline appeals below 4.0 (Turtle Day 3.43); individual-animal turtle-release 5.2→2.4. C2 correlation-abort is moot under C1(b). Penalty −2.5 validated. **Cleared to full re-label.** Selector: `scratchpad/nr_v4_pilot_select.py`.
