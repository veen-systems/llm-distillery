...
| 12 | **Chain 14 non-English**: **ovr#291 — RESHAPED 2026-08-13 (owner): RE-DERIVE, DO NOT REPAIR.** A repairer needs a detector, the detector must *guess*, and the guess is where FS#167's 2,030 false-positive pairs live. A clean copy of every row sits one hop upstream (`original_content`, 0.000%), so nothing need be inferred. U+FFFD (the one irreversibly-lossy class) measured at **4 of 21,316** (474 stored mojibake rows — FS#124's *cause* is fixed), FS#129/#130/#131 language tagging, **NM#231** (the one live stage, now a `human_thriving` v8 criterion) | all | Four repos, one pattern. NM#292's aggregate measurement is **dropped**, not pending; the tracker is retargeted as the stage index plus the per-language-pair-bar constraint. |
