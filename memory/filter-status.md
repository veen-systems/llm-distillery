...
| human_thriving | v8 | 0.539 (gate, calibrated; do NOT rank on this — ADR-023) | 0.6066 test (**worse**, −3.6%) | 6.6K | `jeergrvgreg/human-thriving-filter-v8` (private) | **2026-09-07 DEPLOYED + ENABLED** — NexusMind `9245f2c` (PR #452 shipped the package, **PR #453 enabled it**: #452 alone left it out of `pipeline.enabled_filters`, so nothing called it). ⛔ **First production cycle UNCONFIRMED** — check `data/filtered/human_thriving/` before quoting it as scoring, gpu-server verified by POST: wa 5.604 → medium, `stage_used` stage2, `filter_version` 8.0. ADR-021 gate (EXP-027, epoch 5, CUDA): recall **0.314** / spec **0.9856** / precision 0.550 panel, **0.6073 production-mix** (EXP-028), n=660, 35 positives. Op-point **4.50 calibrated**. ⛔ **Phase E normalization NOT fitted** — `normalization_method: none`, needs ≥200 production rows above 4.5. ⚠️ **`uplifting v7` remains LIVE**: this deploy CREATED human_thriving, it retired nothing, and which one feeds the Thriving tab is an OPEN OWNER DECISION. Adapter sha256 `074209ff…` |
