...
| human_thriving | v8 | 0.539 (gate, calibrated; do NOT rank on this — ADR-023) | 0.6066 test (**worse**, −3.6%) | 6.6K | `jeergrvgreg/human-thriving-filter-v8` (private) | **2026-09-07 DEPLOYED + ENABLED** — NexusMind `9245f2c` (PR #452 shipped the package, **PR #453 enabled it**: #452 alone left it out of `pipeline.enabled_filters`, so nothing called it). ✅ **SCORING CONFIRMED 2026-09-08** — first cycle to score was `filtered_20260907_180414.jsonl`, 6,210 articles, **63 above the 4.50 op-point** (the 12:10 cycle died in image enrichment, #152). ⭐ **First live-output audit (`EXP-029`, $0.16, n=62): scope precision 0.9677 [0.890, 0.991] vs the gate's 0.850, not distinguishable; only 2 of 62 off-lens. The defect is the PROMPT, not the student — swapping it for the documented rubric moves 12 articles out of `in_scope`, paired McNemar p=0.0034, and those 12 are STATUS.md's three owed v8.1 gaps (#153).** ⛔ Phase E is **168/200 cumulative** (~0.9 cycles) — do not disable v8 while it is pending. ADR-021 gate (EXP-027, epoch 5, CUDA): recall **0.314** / spec **0.9856** / precision 0.550 panel, **0.6073 production-mix** (EXP-028), n=660, 35 positives. Op-point **4.50 calibrated**. ⛔ **Phase E normalization NOT fitted** — `normalization_method: none`, needs ≥200 production rows above 4.5. ⚠️ **`uplifting v7` remains LIVE**: this deploy CREATED human_thriving, it retired nothing, and which one feeds the Thriving tab is an OPEN OWNER DECISION. Adapter sha256 `074209ff…` |
