...
**Per-filter prefilters: DECLARED but NOT ENFORCED in production (NM#284, found 2026-08-01).** Every filter's `config.yaml` says `prefilter: enabled: true`, but the GPU scorer builds each with `use_prefilter=False` and calls `score_batch(skip_prefilter=True)` — so no filter's `apply_filter()` has run in the production scoring path since `66582e7` (2026-02-10), and `passed_prefilter` is stamped `true` on 100% of rows. Unaffected and working: the commerce prefilter (ADR-004), the obituary/violence gates, and the same prefilters in the llm-distillery oracle/training path. NM#284 stage 1 (shadow measurement, deployed 2026-08-01 `5d53774`) now logs observed vs declared pass rate per batch. First cycle:
