...
| human_thriving | v8 | 0.539 (gate, calibrated; do NOT rank on this — ADR-023) | 0.6066 test (**worse**, −3.6%) | 6.6K | `jeergrvgreg/human-thriving-filter-v8` (private) | **2026-09-07 DEPLOYED + ENABLED** — NexusMind `9245f2c` (PR #452 shipped the package, **PR #453 enabled it**: #452 alone left it out of `pipeline.enabled_filters`, so nothing called it). ✅ **SCORING CONFIRMED 2026-09-08** — first cycle to score was `filtered_20260907_180414.jsonl` <!-- placeholder -->, 6,210 articles, **63 above the 4.50 op-point** (the 12:10 cycle died in image enrichment, #152). ⭐ **First live-output audit (`EXP-029`, $0.16, n=62): scope precision 0.9677 [0.890, 0.991] vs the gate's 0.850, not distinguishable; only 2 of 62 off-lens. The defect is the PROMPT, not the student — swapping it for the documented rubric moves 12 articles out of `in_scope`, paired McNemar p=0.0034, and those 12 are STATUS.md's three owed v8.1 gaps (#153).** ⛔ Phase E's rows ARRIVED — **202/200 cumulative** at the 2026-09-08 09:30 cycle — and the fit is refused by **our own guard (#154)**, not by the data: `fit_normalization.py`'s #205 check compares `sample_min` (4.5069) against an absolute 4.5 that IS this filter's op-point. Do not disable v8 while it is pending. ADR-021 gate (EXP-027, epoch 5, CUDA): recall **0.314** / spec **0.9856** / precision 0.550 panel, **0.6073 production-mix** (EXP-028), n=660, 35 positives. Op-point **4.50 calibrated**. ⛔ **Phase E normalization NOT fitted** — `normalization_method: none`; the ≥200 bar is MET (202) and #154 blocks the write. ⚠️ Fitting it also arms NM#319: only **60.4%** (122/202) of surfaced rows would clear NexusMind's normalized 4.0 enrichment gate, against 100% today. ⭐ **v7-vs-v8 on the same 15,372 production articles (`EXP-030`, $0): v7 surfaces 1,184, v8 surfaces 168** — a 7× cut, both 152 / v7-only 1,032 / v8-only 16, and volume-matching v8 does NOT reconstruct v7's set (57.0% at parity, Spearman 0.224 within the surfaced union). ⚠️ **`uplifting v7` remains LIVE**: this deploy CREATED human_thriving, it retired nothing, and which one feeds the Thriving tab is an OPEN OWNER DECISION. Adapter sha256 `074209ff…` |
