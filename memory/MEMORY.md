...
- `climate_doom` cap is a dormant safety net under v4, not load-bearing — the five #161 articles rescore to raw 0.36–1.89 under v4, below both the op-point and the cap's 2.0 ceiling. Override window widened to full body; triggers stay lede-bound (NexusMind `8681efa`). The fit threshold keeps doom off the lens, not this cap. <!-- verify: grep -q 'override_text' ~/repos/veen-systems/NexusMind/src/scoring/cap_triggers.py && echo PASS || echo FAIL -->
...
**⏳ FIRST THING: confirm the 2026-07-14 deploy actually landed.** The `climate_doom` override-window fix (NexusMind `8681efa`) was pushed but NOT hand-deployed — the scorer restart happens legitimately inside `nexusmind.service`'s `ExecStartPre=deploy_filters.sh` on the next pipeline cycle (fluxus timer, every 4h). Running `deploy_filters.sh` manually would start the scorer outside a run and starve ollama of the GPU (see [[feedback-scorer-always-rest]]). Check:
```bash
ssh gpu-server 'cat ~/NexusMind/filters/CODE_REVISION'   # expect 3bbfcf93…
```
If it still shows `dba0fb5d…` after a cycle has run, the deploy did NOT happen — investigate `journalctl -u nexusmind.service` on sadalsuud. Live signal the fix worked: the two Spanish conservation articles (Chile seed-banking, Ecuadorian vizcacha) stop showing `cap_applied: ['climate_doom']` and keep raw 3.79 / 4.28 instead of being flattened to 2.0. Full detail: **`memory/project_session_2026_07_14.md`**.
