...
**⚠️ #62 leakage monitoring is 6 WEEKS OVERDUE — do it before any cd v6 talk.** Scheduled 2026-05-31 → 2026-06-07 and never executed. cd v5 shipped specifically to fix #62 discovery-lens leakage, and **nobody ever confirmed it held in production** — that question is currently unanswered, not answered "no". Cheap: the five named examples (Pope apology, Belgium/Congo, Modigliani repatriation, residential schools, Antwerp Congolese memorial) should score < 4.5; cd v5 output is on sadalsuud at `data/filtered/cultural_discovery/`. Any leak → capture in `datasets/raw/cd_v6_leakage_candidates.jsonl`. This is the check that decides whether cd v6 (`#23 evidence_quality`) is real work or a note. Same shape as every 2026-07-14 finding: a verification written down and never run. **NB: cultural_discovery is NOT otherwise a priority — it is the *reference* solutions v4 copies, not a retrain target.**
