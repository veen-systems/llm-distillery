...
- **Deploys ship the git-archive of HEAD (Fix B, 2026-07-17)** — filter packages MUST be committed+pushed to deploy; a file-copied-but-uncommitted package BLOCKS the every-4h cycle (fail-closed, fires the ntfy alert). Manual deploys race the 4h timer (see gotcha-log 2026-07-17) — prefer pull-and-let-ExecStartPre-run.
...
- **`nexusmind.service` failures now alert** via `OnFailure=nexusmind-alert@` → ntfy.sh (topic in sadalsuud's gitignored `config/credentials/ntfy_topic`) + `data/alerts.log` on sadalsuud.
...
- if [running a review at all], then [run TWO rounds — round 1 reviews the code, round 2 reviews the fixes, and 4 of round 2's 8 findings were defects inside round 1's fixes (including the test meant to catch them)] — promoted from gotcha-log 2026-07-14 (pattern held a SEVENTH consecutive time 2026-07-17: Fix B's 3-model round-5 battery found 7 verified defects in the freshly harness-verified rewrite)
...
## Next Session Pickup (updated 2026-07-17 afternoon)

**✅ Fix B DONE end-to-end 2026-07-17 afternoon — the deploy-hardening arc is CLOSED.** Both
...
with deploy work. Prompt drafting first (`filters/sustainability_technology/v4/prompt-compressed.md`,
7 dims scaffolded, calibration batch spec ready ~$0.30); this is the ADR-020 validation case —
follow cd v5's playbook end-to-end. Schedule the full oracle batch off-peak (after ~noon CEST,
`memory/oracle-pricing-scheduling.md`). Also now unblocked: **#72** nature_recovery v4
normalization refit via production-representative historical rescore (playbook §6) — Fix A
anchoring + Fix B deploy gates both done, sparse fits anchor instead of false-failing.

**⚠️ Engineer actions pending (from 2026-07-17 afternoon):**
1. **Subscribe to the alert topic** — `https://ntfy.sh/$(cat sadalsuud:~/local_dev/NexusMind/config/credentials/ntfy_topic)`
   (topic name was shown in-session; browser or ntfy app). Until subscribed, alerts land only in
   `data/alerts.log`.
2. **sadalsuud `config/app.yaml` local drift**: `notifications.healthcheck.enabled: true → false`
