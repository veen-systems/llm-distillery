...
- Solutions broadening v4 — CALIBRATION BATCH DONE 2026-07-17 evening. Forks signed off (2026-05-05): C (broaden ST v3 in place), combine ST v3 + foresight v1 corpora, foresight retired when v4 supersedes ST v3. Prompt drafted + review-hardened (2 rounds); 350-article batch scored by DeepSeek + Gemini (ADR-020 method, $1.00); both judges → DeepSeek. Awaiting engineer sign-off on 4 decisions (see `filters/solutions/v4/calibration_report.md`) before the ~$10-15 corpus re-score. <!-- verify: test -f filters/solutions/v4/prompt-compressed.md && test -f filters/solutions/v4/calibration_report.md && echo PASS || echo FAIL -->
...
`filters/solutions/v4/calibration_report.md` "Open engineer decisions":
