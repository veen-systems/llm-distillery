...
- [2026-07-22](project_session_2026_07_22.md) — **Solutions v4 DEPLOYED & LIVE-SCORING (cutover EXECUTED, first cron 9,334 scored).** Op-point **2.25** (gate recall 0.559/prec 0.768/F1 0.647); `score_scale_factor`→1.0; `normalization.json` fitted (40K rescore); Hub `solutions-filter-v4` published (DeepSeek). **3 review rounds (15+31+7 confirmed, 15 defects-in-fixes in R2)**: reverted fail-closed guard; added `solutions` across ovr pipeline incl. `summarize.ts`; fixed nr v4 + cd v5 Hub cards. NexusMind `61ecc10` + ovr `c279dc4` + llm-distillery all pushed. **LIVENESS ✅, content-quality NOT validated** (policy bleed, branded bleed, belonging drift) — ADR-020 still DRAFT. Sibling: **nr v4 runs raw-passthrough (#72)**.
...
**✅ Solutions v4 cutover EXECUTED 2026-07-22 evening** (NexusMind `61ecc10` + ovr `c279dc4`). First cron scored 9,334 articles; LIVENESS verified, content-quality gate still owed before ADR-020 Accept. Foresight/sustech retired in config; dirs kept for rollback, delete ~2026-08-01.
...
**🎯 SOLUTIONS v4 (#43) — DEPLOYED & LIVE-SCORING 2026-07-22; quality gate OWED before ADR-020 Accept.** Op-point 2.25, normalization fitted (40K rescore), Hub published (DeepSeek). Replaces sustech-v3 + foresight-v1. LIVENESS verified (9,334 scored, sane multi-tier) but content-quality flagged (policy bleed, branded bleed, belonging drift). **Do full-content quality gate + oracle re-sample before ADR-020 Accepted.** Recall ceiling ~0.58 structural (access-bias → v2).
...
- **foresight v1** — PARKED (#43, 2026-04-16). Captures governance solutions, not foresight; merged into solutions v4 2026-07-22. <!-- verify: grep -qE "\*\*foresight\*\*.*PARKED" CLAUDE.md && echo PASS || echo FAIL -->
