...
8. **The LD#85 reactivation condition has now fired twice in one day, and these two FNs sit far higher than addendum 7's cases — though the FN population as a whole sits *lower*, see the third bullet.** Two more obituaries reached ovr.news on 2026-08-01 and the owner flagged both ("this is an obituary" / "Obituary"). Scores are the **production stamps** read straight out of the NexusMind filtered batches — no rescore, so no reproduction step is needed:
   ...
   | --- | --- | --- | --- | --- |
   | "'A person of faith and courage': Carol Lynn Pearson's legacy of compassion" (Deseret) | en | **0.6866** | belonging, raw 6.93 → norm 9.49, tier high | `belonging/filtered_20260801_085409.jsonl` <!-- placeholder --> |
   | "Simone Forti's Dance of Life" (Hyperallergic) | en | **0.5783** | cultural_discovery, raw 5.41 → norm 7.81, tier high | `cultural_discovery/filtered_20260801_004817.jsonl` <!-- placeholder --> |
...
9. **The consumer solved it downstream, and that relocates the problem rather than closing it.** ovr.news now **flags** articles whose *own generated summary* announces a death (`src/lib/data/editor/rules/obituary-summary.ts`). It **enforces** since 2026-08-02, per that repo's ADR-041: a rule may remove articles only once its removals are durably recorded, which `scripts/editor-decisions-snapshot.ts` now does each cycle. ADR-042 sets the tie-break — editorial uncertainty resolves toward exclusion, because the reader never knows what they didn't see. It catches every case above — including Yves Devos and Teresa Alonso, which this detector cannot reach at any threshold — at a measured **0.74% decision rate over 3,658 build-eligible lens-rows** (27 matches), hand-checked. 25 of the 27 are on the `belonging` lens — 3.0% of it, against 0.1% on thriving and discovery.
