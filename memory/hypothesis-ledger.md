...
| **H-V8-8** | the k=3 repeat discount survives at corpus scale | ⏳ **OPEN — this is an ASSUMPTION carrying a ~$15 decision.** Runs 2–3 cost $0.000266/article because the identical prompt returned ~1 minute later. A 6,590-row pass takes ~30 min, and **DeepSeek's cache TTL has never been measured**. ⛔ **METHOD SUPERSEDED 2026-08-29** (`docs/evidence/2026-08-29-v8-cache-ttl/PREREGISTRATION.md`): *re-score the same 50* lets the ARTICLE text cache too, which a corpus pass never gets — every article there is new. Replaced by **four DISJOINT 50-row sets at t+0/30/60/90**, so a hit can only be the shared prefix. ⭐ **First result: the prefix was still hot after 77.9 min of inactivity (P0, 0 full-prefix misses in 50 rows), which reframes the question — a corpus pass CALLS CONTINUOUSLY and never lets the prefix cool.** **Or make it moot:** schedule k=3 as three back-to-back calls *per article* rather than three passes. **Revisit trigger:** before any corpus-scale k≥3 run is costed. |
