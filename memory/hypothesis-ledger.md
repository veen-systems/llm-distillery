...
| **H-V8-9** | the reorder's stricter labels are BETTER, not merely different | ⏳ **STILL OPEN — evidence gathered 2026-08-29, $0 (re-read of labels already bought), and the ruling is the owner's.** All **12** rows whose op-point SIDE differs at k=3 were adjudicated against the v8 prompt's own STEP 1 / §5: `docs/evidence/2026-08-29-v8-h-v8-9-adjudication/`. **A(reordered) surfaces 15, B(as-is) 23** — reconciles exactly with the k=3 counts in the Phase A `results.txt` §3, so the set is not hand-built. ⭐ **Only 4 of the 12 are STABLE in both arms** (every run agreeing with its own arm); 2 more cross because a SINGLE `out_of_scope` run drags the k=3 mean over — #135, not the reorder — and the remaining 6 are magnitude drift with all six runs `in_scope`, five drops of 0.18–0.73. On the 4 stable ones the reorder is right **3 of 4**: it drops a 2060 projection study, an ODA funding plan and a mentorship guidebook — the §5-speculation, money-committed and class-B/#125 shapes v8 exists to remove, each scored `in_scope` by as-is on 3/3 runs. ⛔ **Its one stable ADD is a false positive**: the WestJet harassment settlement, where the prompt's *'compensation was paid'* guard does not apply — the article says a claims portal *is being prepared*. **The reorder is not uniformly stricter at the gate, it is differently strict.** Cost side: 2, at most 3, true positives lost (Thai children learning Khon; Feijenoord's 200th; Seoul's 540 independence activists, honours *applied for*), all within **0.24** of the op-point. ⛔⛔ **THE HOLE, and it is the reason this stays OPEN: neither §5b hazard could have fired — the three no-regression articles are not in this 200-row draw, so the cohort could not produce that positive.** **Method to close:** score the 3 no-regression rows under both arms at k=3 (~$0.03) and check criterion 2 per row against its own `assertion`. **Revisit trigger:** before adopting the reordered prompt for Phase B. ⛔ Do not resolve this from the price. |
