...
class CulturalDiscoveryPreFilterV6(BasePreFilter):
    """Fast rule-based pre-filter for cultural discovery content.

    Declarative-shape exclusion data per ADR-018; custom apply_filter retained
    for per-category exceptions and URL-based domain blocking.

    v6.0 carries v5's rules UNCHANGED — including the LD#86 multilingual topic
    gate. That is deliberate: #98's acceptance criteria compare the e5 probe
    against this gate on identical rows, so the gate must stay byte-identical
    until the probe beats it (criteria 1-3). Stripping the gate and moving to
    the pure BasePreFilter declarative shape is criterion 4, i.e. after the
