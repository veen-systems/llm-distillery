"""
Human Thriving Filter v9 - Local Inference Pipeline

Pipeline: Article -> Model -> Gatekeeper -> Calibration -> Tier

⛔ NO PREFILTER STAGE. v9 ships no per-lens keyword prefilter (ADR-018/019
...
`~/llm-distillery/filters/human_thriving/v9/model/`. Constructing this scorer without
...
Usage:
    ...
    python filters/human_thriving/v9/inference.py --input articles.jsonl --output out.jsonl
"""
...
from filters.human_thriving.v9.base_scorer import BaseHumanThrivingScorer
...
class HumanThrivingScorer(BaseHumanThrivingScorer):
    """
    Local-files scorer for human_thriving v9.
...
def main():
    ...
    parser = argparse.ArgumentParser(
        description="Score articles with the human_thriving v9 filter"
    )
