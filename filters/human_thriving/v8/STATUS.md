...
✅ **Phase E FITTED 2026-09-22** on **2,976** rows, `raw_min == op_point == 4.5` exactly.
`#154` — the guard that refused the 202-row fit, comparing `sample_min` against an absolute
4.5 that IS this filter's op-point — was ruled (option 1: test the gap, not the bound) and
fixed. ⛔ **Fitting is NOT a cutover**: `uplifting v7` still scores, and this package is
**not yet deployed with the file** — it reaches production only via `deploy_to_nexusmind.sh`
→ NexusMind commit → `deploy_filters.sh`. Labelled, adjudicated; prompt settled at
v8.4. ⭐ **The v7-vs-v8 same-articles comparison has RUN** (`EXP-030`, $0, `#151`): on the same
15,372 production articles v7 surfaces **1,184** and v8 **168** — a 7× cut, and volume-matching v8
does not reconstruct v7's set. Last updated 2026-09-08.
...
| Phase E normalization | ✅ **FITTED 2026-09-22 on 2,976 rows** (`#154` ruled + fixed; record: `docs/decisions/2026-09-22-phase-e-fit-and-the-154-guard.md`). ⚠️ The 60.4% below was **in-sample and near-tautological and the 2,976-row refit does NOT repair that** — 60.0% on the fitted rows is the same identity at larger n. The transferable number is the effective **raw** bar, **4.794**. The prior state, kept because its reasoning still applies: `fit_normalization.py`'s #205 check compares `sample_min` (4.5069) against an absolute `MAX_NORMALIZATION_RAW_MIN = 4.5` that IS this filter's op-point, so at a 4.5 op-point it is a sample-DENSITY test; `tests/unit/test_normalization_invariant.py:190` carries the same bound, so a fitter-only fix leaves the file uncommittable. ⚠️ Fitting also arms NM#319: a projection over the 202 rows puts **60.4%** above the normalized 4.0 enrichment gate against 100% today — ⛔ in-sample and near-tautological (normalized 4.0 is those rows' own 40th percentile), so read it as shape, not as a measurement. Until then `normalization_method: none` and raw passes through, so every surfaced article clears the 4.0 enrichment gate. Original reasoning:  — `fit_normalization.py` reads NexusMind production output and needs ≥200 rows above the op-point; sadalsuud has **no `human_thriving`** at all. Normalization comes AFTER deployment, as it did for `solutions v6`. ⛔ Do not substitute the test split: it is a 25.1× design-weighted sample |
