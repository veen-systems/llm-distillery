...
- Consumer product reviews, shopping guides, product launches without a problem-solving deployment story. **This includes standalone model evaluations, benchmark comparisons, and "best X of 202X" tech roundups:** the PRODUCT may be a solution, but the ARTICLE is a review — score both as `not_a_solution`. *Test:* Delete the evaluation/comparison framing. Is there still an article about a deployed solution? If NO, out of scope. *Contrastive pair:* "We benchmarked 5 carbon-capture startups — here's who leads" = OUT of scope (the article is the benchmark, not the solution). "Company X's carbon-capture unit removed 10K tonnes last year, according to independent audit" = IN scope (the article reports the deployment, even if it also compares).
- General science research with no application pathway stated. **A discovery, lab result, or dataset is not a solution.** Score `not_a_solution` if: the article centers on a scientific FINDING (record efficiency, genome sequenced, specimen analyzed, algorithm achieves SOTA) rather than on a solution DEPLOYMENT. The discovery itself is the news, not a problem being solved. *Test:* Is there a named organization or population USING this discovery to solve a problem TODAY, with measurable outputs? If NO, out of scope. *Contrastive pair:* "Perovskite cell hits 26% efficiency in lab" = OUT of scope (the finding is the news; no deployment). "Perovskite manufacturer ships first 10 MW of 26%-efficient panels to solar farm" = IN scope (deployed, named adopter, measurable output).
...
- A government announces a new law, regulation, ban, subsidy change, or enforcement action → Flag A (regulation ≠ solution unless the article demonstrates measurable outcomes from the mechanism). **ENFORCEMENT includes fines, sanctions, antitrust penalties, and court-ordered remedies.** The regulator acting is NOT the same as the regulated problem being solved — a €890M antitrust fine punishes past conduct; it does not deploy a solution. *Contrastive pair:* "EU fines Google €890M for antitrust violation" = Flag A (enforcement action, no solution deployed). "EU's Digital Markets Act: one year of interoperable messaging for 450M users" = IN scope (implemented regulation with measured outcomes).
